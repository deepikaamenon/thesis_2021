...
vps1Δ cells show a slight growth defect at 37C, as has been reported21. In vps1Δ cells, Sla1 accumulates in patches at the plasma membrane, moves inwards, and disassembles like in WT. vps1Δ does not increase the rate of membrane retraction (Fig.2.5C). 
	...
	{A: Dot spots of yeast cells in WT, Vps1-GFP (diploid), and vps1Δ cell at 30C and 37C. 
		...
		C: Failure rate of membrane scission, in vps1Δ, rvs167Δ and WT cells. 
		\label{fig4_vpsdel1}}
\end{figure}
...
\vspace{5mm}
	...
	\caption[Rvs localization in vps deletion]
	{D, E: Averaged centroid movement and normalized fluorescent intensity of Sla1-GFP in WT and \textit{vps1$\Delta$} cells. Time =0 (s) for WT Sla1 centroid is scission time. Sla1 for \textit{vps1$\Delta$} is shifted in time to begin inwards movement at the same time as WT. 
		F, G: Averaged centroid movement and normalized fluorescent intensity of Rvs167-GFP in WT and  \textit{vps1$\Delta$} cells. Time =0 (s) for WT Rvs167 centroid is aligned to scission time. Rvs167 for \textit{vps1$\Delta$} is shifted in time so that fluorescent intensity maxima is at time=0 (s).
		\label{fig4_vpsdel2}}
\end{figure}
...
\begin{figure}[H]
	...
	{A: Maximum intensity projections of cells expressing GFP-tagged Inp51, Inp52, and Inp53. Exposure time 80ms.
		...
		C: Maximum intensity projection of Sla1-GFP in WT,  \textit{inp51$\Delta$},  \textit{inp52$\Delta$} cells. Rvs167-GFP in WT,  \textit{inp51$\Delta$},  \textit{inp52$\Delta$} and  \textit{inp51$\Delta$}\textit{inp52$\Delta$}   cells.
...
{A: Movement of Sla1 in WT, \textit{inp51$\Delta$ }  and \textit{inp52$\Delta$ } cells. Time=0 (s) for WT strains corresponds to scission time. Sla1 centroids for \textit{inp51$\Delta$ }, \textit{inp52$\Delta$ }  are shifted to move inwards at the same time as WT. 
...
Dynamics of Rvs disassembly are quite different. Fig.2.8B shows that disassembly is slowed by ~1.5 seconds in 2xh compared to 1xh cell. In the corresponding Rvs centroid movement traces (Fig.2.8C), instead of the sharp jump seen in WT, there is a delay in movement into the cytoplasm.
				...
				\caption[Overexpression of the Rvs complex in haploid cells]
					{A. Movement Sla1-GFP in diploid strains consisting of 1 (WT, 1xh), 2 copies (2xh) of the Rvs161, Rvs167 genes in haploid cells. Sla1-GFP for 1xh is aligned to that time=0 (s) corresponds to scission time. 2xh is shifted to move inwards at the same time. 
...
\newpage
						...
						{A. Movement of Sla1-GFP in diploid cells consisting of 1 (2xd), 2 (WT, 2xd) and 4 copies (4xd) of Rvs161, Rvs167 genes. Sla1-GFP for 2x and 4x copies of Rvs are aligned so that Time=0 (s) corresponds to scission time. Sla1-GFP for 1x strain is shifted to move inwards at the same time as the other two.
						B. Rvs167 movement in cells containing 1, 2, and 4 copies of Rvs genes. Each is shifted so that time=o (s) corresponds to its fluorescent intensity maximum.
						C.  Fluorescent intenties of Rvs167-GFP in cells containing 1, 2, 4 copies of Rvs. Alignment is same as Fig.2.9B.
...
This shows that shallow invaginations of the rvs167Δ can be rescued by recruiting only BAR domains of Rvs167.
	...
	%	\centering
		...
		{A: Movement of Sla1 in haploid cells consisting of 1 (WT: 1xh) and two copies (2xh) of Rvs genes, 1 (1xBAR) and 2 copies of BAR domain ( 2xBAR) and  \textit{rvs167$\Delta$} cells. Time=0 (s) for WT Sla1-GFP corresponds to scission time. Other centroids have been shifted to move inwards at the same time as the WT.
		B: Movement of Rvs167-GFP in 1xh, 2xh, 1xBAR, 2XBAR cells.
		C:  Maximum molecule number and standard error of mean of Rvs167 recruited to endocytic sites in the respective cells. P-values from two-sided z test, * = p$\leq$ 0.05, ** = p$\leq$ 0.01, *** = p$\leq$ 0.001. }
		\label{fig_scaffold}
		\end{figure}
...
This shows that the Rvs scaffold does not serve to counter turgor pressure.  
	...
	\begin{figure}[H]
	\centering
	\includegraphics[width=15cm,height=15cm,keepaspectratio]{figures/results_final/sorbitol2}
	\caption[Treatment of Rvs167del with Sorbitol]
	{Movement of Sla1-GFP in WT, WT cells treated with sorbitol, Rvs167-GFP in WT and WT cells treated with sorbitol}
	\end{figure}

\newpage
\begin{table}[H]
	\centering
	\includegraphics[width=14cm,height=25cm,keepaspectratio]{figures/results_final/table_numbers}
